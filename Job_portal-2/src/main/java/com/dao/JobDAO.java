...
public class JobDAO {
	...
	public JobDAO(Connection conn) {
	 ...
	 public boolean addJobs( Jobs  j)
	 {
		 ...
		 try {
			 String sql="insert into place(title,description,location,category,status)values(?,?,?,?,?)";
...
public Jobs getJobById(int id ) {
	 ...
	 public  boolean updateJob(Jobs j) {
		 ...
		 try {
			 String sql="update  place  set title=?,description=?,location=?,category=?,status=? where id=?";
			 PreparedStatement ps=conn.prepareStatement( sql );
			 ps.setString(1, j.getTitle());
			 ps.setString(2, j.getDescription());
			 ps.setString(3, j.getLocation());
 ...
 public List<Jobs> getJobsORLocationAndCate(String category, String location){
	...
	Jobs j=null;
	 try {
		 String sql = "select * from place where category=? or location=? order by id DESC";
		 PreparedStatement ps = conn.prepareStatement(sql);
 ...
 public List<Jobs> getJobsAndLocationAndCate(String category, String location){
		...
		Jobs j=null;
		 try {
			 String sql = "select * from place where category=? and location=? order by id DESC";
			 PreparedStatement ps = conn.prepareStatement(sql);
			 ps.setString(1, category);
			 ps.setString(2, location);
			 ResultSet rs =ps.executeQuery();
